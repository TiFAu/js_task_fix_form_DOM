...
function addingPlaceholderAndLabel(element, index, arr) {
    ...
    if (element.name) element.placeholder = element.name.charAt(0).toUpperCase() + element.name.slice(1);
    elementLabel.className = 'field-label';
    if (!element.id && element.name) elementLabel.htmlFor = element.name;
